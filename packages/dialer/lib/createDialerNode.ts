...
const createDialerNode = async (config: PartialRoseNetNodeConfig) => {
  ...
  // const runSubscribeCallback = async (channel: SubscribeChannel, receivedData: ReceiveDataCommunication, from: string) => {
  //     hasUrl(channel)
  //         ? channel.func(
  //             receivedData.msg,
  ...
  const handleIncomeMessage = async (from: string, message: string) => {
    try {
      ...
      // } else {
      //     logger.debug(
      //         `Received a message from [${from}] in unsubscribed channel [${
      //             receivedData.channel
      //         }].`
      //     );
      // }
  ...
  const sendMessage = async (
    ...
    if (receiver) {
      ...
      node.publish(DEFAULT_PUBSUB_TOPIC, stringifyData);
    }
  };
  ...
  return {
    ...
    getPeerIds: node.info.getConnectedPeers,
    _node: node,
  };
};
